...
describe('useAuth', () => {
    ...
    it('should restore user from localStorage on mount', () => {
        ...
        localStorage.setItem('xalabaas_user', JSON.stringify(storedUser));
    ...
    it('should sign in with password via Convex mutation', async () => {
        ...
        vi.mocked(useCachedMutation)
            .mockReturnValueOnce(mockSignIn as never)      // signInWithPassword
            .mockReturnValueOnce(mockDemo as never)        // signInAsDemo
            .mockReturnValueOnce(mockDeleteSession as never); // deleteSession
        vi.mocked(useCachedAction).mockReturnValue(vi.fn() as never);
        vi.mocked(useCachedQuery).mockReturnValue(undefined as never);
    ...
    it('should sign out and clear localStorage', async () => {
        const storedUser = { id: 'u1', email: 'test@example.com', name: 'Test' };
        localStorage.setItem('xalabaas_user', JSON.stringify(storedUser));
        localStorage.setItem('xalabaas_session_token', 'token');
        ...
        expect(result.current.isAuthenticated).toBe(false);
        expect(result.current.user).toBeNull();
        expect(localStorage.getItem('xalabaas_user')).toBeNull();
        expect(localStorage.getItem('xalabaas_session_token')).toBeNull();
    });
