...
/**
 * Helper: Search through an array of objects with a specific field
 */
function searchObjectArray(arr: any[] | undefined, field: string, term: string): any[] {
    if (!arr || !Array.isArray(arr)) return [];
    return arr.filter(item => item && (containsTerm(item[field], term) || isFuzzyMatch(item[field] || "", term)));
}
...
export const globalSearch = query({
    ...
    handler: async (ctx, args) => {
        ...
        if (term.length < 1) {
            return { results: [], total: 0, hasMore: false, categorySuggestions: [] };
        }

        // Get smart category suggestions based on the search term
        const categorySuggestions = includeCategorySuggestions
            ? getMatchingCategories(term)
            : [];
...
export const typeaheadPublic = query({
    ...
    handler: async (ctx, { prefix, categoryKey, limit = 10, includeCategorySuggestions = true }) => {
        ...
        const matches = resources
            .map((r: any) => {
                ...
                if (meta?.city) {
                    const normalizedCity = normalizeNorwegian(meta.city.toLowerCase());
                    ...
                    } else if (isFuzzyMatch(meta.city, term)) {
                        score = Math.max(score, 30);
