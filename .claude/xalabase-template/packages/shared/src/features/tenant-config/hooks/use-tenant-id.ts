/**
 * XalaBaaS SDK - Tenant ID Resolution Hook
 *
...
import type { TenantId } from "../../_shared/hooks/convex-api";

/** Known app IDs whose localStorage keys we scan when no specific appId is given. */
const KNOWN_APP_IDS = ["web", "backoffice", "minside", "saas-admin", "default"] as const;
...
export function useSessionTenantId(appId?: string): TenantId | undefined {
    return useMemo(() => {
        try {
            ...
            if (appId) {
                const perApp = localStorage.getItem(`xalabaas_${appId}_tenant_id`);
                if (perApp) return perApp as TenantId;
            }

            // 2. Scan all known app keys
            for (const knownId of KNOWN_APP_IDS) {
                const val = localStorage.getItem(`xalabaas_${knownId}_tenant_id`);
                if (val) return val as TenantId;
            }

            // 3. Legacy key
            const legacy = localStorage.getItem("xalabaas_tenant_id");
            ...
            for (const id of appIds) {
                const userJson = localStorage.getItem(`xalabaas_${id}_user`);
                if (userJson) {
            ...
            // 5. Legacy user key
            const legacyUser = localStorage.getItem("xalabaas_user");
            if (legacyUser) {
