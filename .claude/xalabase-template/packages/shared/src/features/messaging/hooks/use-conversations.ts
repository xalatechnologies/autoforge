/**
 * XalaBaaS SDK - Conversation & Messaging Hooks (Tier 2)
 *
...
export function useConversations(
  ...
  const conversations: Conversation[] = (data ?? []).map((c) => ({
    ...
    participants: c.participants.map((p: any) => p as string),
    subject: c.subject,
    status: c.status,
...
export function useConversation(id: Id<"conversations"> | undefined) {
  ...
  const conversation: Conversation | null = data
    ? {
      ...
      participants: data.participants.map((p: any) => p as string),
      subject: data.subject,
      status: data.status,
      unreadCount: data.unreadCount,
      lastMessageAt: data.lastMessageAt ? new Date(data.lastMessageAt).toISOString() : undefined,
...
export function useMessages(
  ...
  const data = useConvexQuery(
    ...
    conversationId ? { conversationId, limit: params?.limit } : "skip"
  );
...
/** @deprecated Use useUnreadMessageCount instead */
export const useUnreadCount = useUnreadMessageCount;

/** @deprecated Use useMarkMessagesAsRead instead */
export const useMarkMessagesRead = useMarkMessagesAsRead;
