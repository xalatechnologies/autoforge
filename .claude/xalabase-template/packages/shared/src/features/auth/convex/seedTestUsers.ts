 ...
 * - Backoffice users (admin, saksbehandler, manager, aktor): get tenantId
...
const TEST_USERS = [
    ...
    },
    {
        ...
        demoToken: "porsgrunn-admin-001",
        needsTenant: true, // Backoffice user
    },
    {
        email: "aktor-admin@test.xala.no",
        nin: "01028731015",
        phoneNumber: "91138813",
        role: "admin",
        description: "Aktor Admin",
        needsTenant: true, // Backoffice user
    },
    {
        email: "aktor-utleier@test.xala.no",
        nin: "21090295842",
        phoneNumber: "98393410",
        role: "manager",
        description: "Aktor Utleier",
        needsTenant: true, // Backoffice user
    },
    {
        email: "aktor-saksbehandler@test.xala.no",
        nin: "09013039841",
        phoneNumber: "47030508",
        role: "saksbehandler",
        description: "Aktor Saksbehandler",
        needsTenant: true, // Backoffice user
    },
    ...
    // ============================================================================
    {
        email: "bankid-bruker@test.xala.no",
        nin: "15860771346",
        role: "user",
    ...
    {
        email: "bankid-saksbehandler@test.xala.no",
        nin: "06881271913",
        role: "saksbehandler",
    ...
    {
        email: "bankid-admin@test.xala.no",
        nin: "30916326773",
        role: "admin",
    ...
    {
        email: "bankid-aktor-admin@test.xala.no",
        nin: "03852358504",
        role: "admin",
        description: "BankID Aktor Admin",
        needsTenant: true, // Backoffice user
    },
    {
        email: "bankid-aktor-utleier@test.xala.no",
        nin: "13891199915",
        role: "manager",
        description: "BankID Aktor Utleier",
        needsTenant: true, // Backoffice user
    },
    {
        email: "bankid-aktor-saksbehandler@test.xala.no",
        nin: "16837147593",
        role: "saksbehandler",
        description: "BankID Aktor Saksbehandler",
        needsTenant: true, // Backoffice user
    },
    ...
    // ============================================================================
    {
        email: "demo@xala.no",
        role: "admin",
        description: "Xala Demo Admin",
        demoToken: "xala-demo-001",
        needsTenant: true, // Backoffice user
    },
    {
        email: "monitoring@xala.no",
        role: "superadmin",
...
export const associateUsersWithTenant = mutation({
    args: {},
    handler: async (ctx) => {
        // Get the default tenant (Skien Kommune)
        const tenant = await ctx.db.query("tenants").filter((q) => q.eq(q.field("slug"), "skien")).first();
        if (!tenant) {
            return { success: false, error: "No tenant found with slug 'skien'. Run seedsFull:seedFromJson first." };
        }
...
export const seedNinData = mutation({
    ...
    handler: async (ctx) => {
        ...
        // Get or create the default tenant (Skien Kommune)
        const tenant = await ctx.db.query("tenants").filter((q) => q.eq(q.field("slug"), "skien")).first();
